...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case "TOGGLE_STATUS_BUCKET": {
      ...
      return {
        ...state,
        filters: { ...state.filters, statusBuckets: buckets },
      };
    }
    case "TOGGLE_METHOD": {
    ...
    case "SET_HIDE_CACHED":
      return {
        ...state,
        filters: { ...state.filters, hideCached: action.value },
      };
    case "SET_SORT":
      return {
        ...state,
    ...
    case "ZOOM_VIEWPORT": {
      ...
      const newWindow = Math.min(
        totalMs,
        Math.max(MIN_WINDOW_MS, windowMs / action.factor),
      );
    ...
    case "PAN_VIEWPORT": {
      ...
      return {
        ...
        viewport: {
          ...state.viewport,
          startMs: Math.round(newStart),
          endMs: Math.round(newStart + windowMs),
        },
      };
    }
    case "SELECT_SINGLE":
    ...
    case "SET_COLUMN_WIDTHS":
      return { ...state, ui: { ...state.ui, columnWidths: action.widths } };
    default:
      return state;
  }
}
  ...
  return useMemo(() => {
    ...
    if (filters.statusBuckets.length > 0) {
      result = result.filter((r) =>
        filters.statusBuckets.includes(r.statusBucket),
      );
    }
