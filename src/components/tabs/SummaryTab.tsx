...
interface Props { requests: TraceRequest[] }
    ...
    return (
      <div className="p-3 space-y-3">
        <Section title="General">
          ...
          <Row label="Method" value={req.method} color={methodColor(req.method)} mono />
          <Row label="Status" value={String(req.status)} color={statusBgClass(req.status)} />
        ...
        <Section title="Timing Breakdown">
          {Object.entries(req.timing).map(([phase, ms]) => (
            <Row key={phase} label={phase.charAt(0).toUpperCase() + phase.slice(1)} value={formatMs(ms)} />
          ))}
        </Section>
      </div>
    );
  ...
  return (
    <div className="p-3 space-y-3">
      <Section title="Selection Summary">
        ...
        <Row label="Time span" value={`${formatMs(minStart)} – ${formatMs(maxEnd)}`} />
      </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <div className="rounded p-2 space-y-1" style={{ background: 'var(--bg-base)' }}>{children}</div>
    </div>
  );
}
...
function Row({ label, value, mono, color }: { label: string; value: string; mono?: boolean; color?: string }) {
  return (
    <div className="flex items-start gap-2 text-xs">
      <span className="w-24 flex-shrink-0" style={{ color: 'var(--text-muted)' }}>{label}</span>
      <span className={`flex-1 break-all ${mono ? 'font-mono' : ''} ${color ?? ''}`} style={{ color: color ? undefined : 'var(--text-primary)' }}>{value}</span>
    </div>
  );
}
