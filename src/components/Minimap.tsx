...
export function Minimap() {
  ...
  const dragRef = useRef<{
    ...
    dragMode: "pan" | "resizeLeft" | "resizeRight";
  }>({
    dragging: false,
    startX: 0,
    startViewport: { startMs: 0, endMs: 0 },
    dragMode: "pan",
  });
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const W = rect.width;
      const winXPx = (startMs / totalMs) * W;
      const winWPx = ((endMs - startMs) / totalMs) * W;
      const leftEdgePx = winXPx;
      const rightEdgePx = winXPx + winWPx;
      const clickXPx = e.clientX - rect.left;

      // Edge detection threshold
      const edgeThreshold = 8;
      let dragMode: "pan" | "resizeLeft" | "resizeRight" = "pan";

      const onLeftEdge =
        clickXPx >= leftEdgePx - edgeThreshold &&
        clickXPx <= leftEdgePx + edgeThreshold;
      const onRightEdge =
        clickXPx >= rightEdgePx - edgeThreshold &&
        clickXPx <= rightEdgePx + edgeThreshold;
      ...
      if (onLeftEdge) {
        ...
        if (!inWindow) {
          ...
          dispatch({
            type: "SET_VIEWPORT",
            viewport: { startMs: newStart, endMs: newStart + windowMs },
          });
        }
      }
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (dragMode === "pan") {
        // Pan: move entire window
        const windowMs = endMs - startMs;
        newStart = startMs + deltaMs;
        ...
        newStart = startMs + deltaMs;
        newEnd = endMs;
        // Prevent window from inverting and ensure minimum size
        const minWindowMs = Math.max(1, (endMs - startMs) * 0.1); // At least 10% of original
        if (newStart >= newEnd - minWindowMs) {
          newStart = newEnd - minWindowMs;
        }
        if (newStart < 0) newStart = 0;
      } else if (dragMode === "resizeRight") {
        // Resize right edge
        newStart = startMs;
        newEnd = endMs + deltaMs;
        // Prevent window from inverting and ensure minimum size
        const minWindowMs = Math.max(1, (endMs - startMs) * 0.1);
        if (newEnd <= newStart + minWindowMs) {
          newEnd = newStart + minWindowMs;
        }
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full cursor-ew-resize select-none border-b group"
      style={{
      ...
      title="Timeline minimap — drag the highlighted window to pan, or click to jump"
    >
      <canvas
        ref={canvasRef}
